...
// the "Live-fire rounds" and "Logged sessions" tiles. The caption gains a "· N mo" suffix when
// a window is chosen, and clears back on "All time".

test.describe('Home stat range', () => {
  test('the range toggle scopes the Live-fire rounds & Logged sessions tiles', async ({ page }) => {
    await seedDemo(page);
    ...
    await expect(page.getByText('Logged sessions · 12 mo')).toBeVisible();
