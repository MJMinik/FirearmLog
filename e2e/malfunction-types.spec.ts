...
// "Malfunctions" heading; its Reveal toggle is labeled "Log a malfunction"
// (previously it was itself labeled "+ Add Malfunction", which doubled as
// both the section toggle and the row-add button — confusing; a later fix
// then renamed it to duplicate the h2, which created two same-named
// "Malfunctions" controls on desktop next to the sidebar nav link — so it's
// now its own distinct label). On a fresh session the Reveal starts
// collapsed, so every flow here opens it by tapping "Log a malfunction"
// first, then "+ Add Malfunction" to add a row.
...
test.describe('Malfunction details (App 3a)', () => {
  test('round number is saved and reappears when the session is reopened', async ({ page }) => {
    ...
    // Log row opens the editable session form directly. The "Log a malfunction"
    // Reveal opens itself (defaultOpen) because this session already has one.
