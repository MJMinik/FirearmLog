...
test.describe('North Star (pinned goal)', () => {
  test('star pins a goal and echoes it on Home; unstar removes it', async ({ page }) => {
    ...
    await main.getByRole('button', { name: 'Make Draw under 1.2s your North Star' }).click();
    await expect(main.getByText('North Star', { exact: true })).toBeVisible();
    await expect(
      main.getByRole('button', { name: 'Remove Draw under 1.2s as your North Star' }),
    ).toBeVisible();

    // Home echoes it, above Needs Attention.
    await gotoTab(page, 'Home');
    await expect(page.getByRole('main').getByText('Draw under 1.2s')).toBeVisible();
    await expect(page.getByRole('main').getByText(/North Star/)).toBeVisible();

    // Unstar from Progress → Home no longer shows it.
    await gotoTab(page, 'Progress');
    await page
      .getByRole('main')
      .getByRole('button', { name: 'Remove Draw under 1.2s as your North Star' })
      .click();
    await gotoTab(page, 'Home');
